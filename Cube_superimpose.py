...
previous_p = None
prev_x5, prev_y5, prev_z5 = 0, 0, 0
prev_x6, prev_y6, prev_z6 = 0, 0, 0
prev_x7, prev_y7, prev_z7 = 0, 0, 0
prev_x8, prev_y8, prev_z8 = 0, 0, 0
weight = 10
list_KFs = []
for i in range(8):
    KF = Kalman.KF2d(dt=1)
...
# Get projection matrix based on camera calibrations
def projection_matrix(h, calibrate, length):
    global previous_p
    h1 = h[:, 0]
    h2 = h[:, 1]
    lamb = 2 / (np.linalg.norm(np.matmul(np.linalg.inv(calibrate), h1)) + np.linalg.norm(
        np.matmul(np.linalg.inv(calibrate), h2)))
    b_dash = lamb * np.matmul(np.linalg.inv(calibrate), h)

    det = np.linalg.det(b_dash)
    if det > 0:
        b = b_dash
    else:
        b = -1 * b_dash
    ...
    P = np.matmul(calibrate, R)
    return P


# Draw cube on the final output frame
def draw(frame, P, length):
    global prev_x5, prev_y5, prev_z5
    global prev_x6, prev_y6, prev_z6
    global prev_x7, prev_y7, prev_z7
    global prev_x8, prev_y8, prev_z8
    global weight
    ...
    if length < 2:
        ...
        z8 += prev_z8 * weight
        z8 /= (weight + 1)
        prev_x5, prev_y5, prev_z5 = x5, y5, z5
        prev_x6, prev_y6, prev_z6 = x6, y6, z6
        prev_x7, prev_y7, prev_z7 = x7, y7, z7
        prev_x8, prev_y8, prev_z8 = x8, y8, z8
    ...
    for i in range(8):
        z = np.array([x_list[i], y_list[i]], dtype=np.float)
        KF = list_KFs[i]['KF']
        x = list_KFs[i]['x']
        P = list_KFs[i]['P']
    ...
    cv2.line(frame, (int((x1 + fx[0]) / z1), int((y1 + fy[0]) / z1)), (int((x5 + fx[4]) / z5), int((y5 + fy[4]) / z5)),
             (0, 255, 0), 5)
    cv2.line(frame, (int((x2 + fx[1]) / z2), int((y2 + fy[1]) / z2)), (int((x6 + fx[5]) / z6), int((y6 + fy[5]) / z6)),
             (0, 255, 0), 5)
    cv2.line(frame, (int((x3 + fx[2]) / z3), int((y3 + fy[2]) / z3)), (int((x7 + fx[6]) / z7), int((y7 + fy[6]) / z7)),
             (0, 255, 0), 5)
    cv2.line(frame, (int((x4 + fx[3]) / z4), int((y4 + fy[3]) / z4)), (int((x8 + fx[7]) / z8), int((y8 + fy[7]) / z8)),
             (0, 255, 0), 5)
    cv2.line(frame, (int((x1 + fx[0]) / z1), int((y1 + fy[0]) / z1)), (int((x2 + fx[1]) / z2), int((y2 + fy[1]) / z2)),
             (0, 255, 0), 5)
    cv2.line(frame, (int((x1 + fx[0]) / z1), int((y1 + fy[0]) / z1)), (int((x3 + fx[2]) / z3), int((y3 + fy[2]) / z3)),
             (0, 255, 0), 5)
    cv2.line(frame, (int((x2 + fx[1]) / z2), int((y2 + fy[1]) / z2)), (int((x4 + fx[3]) / z4), int((y4 + fy[3]) / z4)),
             (0, 255, 0), 5)
    cv2.line(frame, (int((x3 + fx[2]) / z3), int((y3 + fy[2]) / z3)), (int((x4 + fx[3]) / z4), int((y4 + fy[3]) / z4)),
             (0, 255, 0), 5)
    cv2.line(frame, (int((x5 + fx[4]) / z5), int((y5 + fy[4]) / z5)), (int((x6 + fx[5]) / z6), int((y6 + fy[5]) / z6)),
             (0, 255, 0), 5)
    cv2.line(frame, (int((x5 + fx[4]) / z5), int((y5 + fy[4]) / z5)), (int((x7 + fx[6]) / z7), int((y7 + fy[6]) / z7)),
             (0, 255, 0), 5)
    cv2.line(frame, (int((x6 + fx[5]) / z6), int((y6 + fy[5]) / z6)), (int((x8 + fx[7]) / z8), int((y8 + fy[7]) / z8)),
             (0, 255, 0), 5)
    cv2.line(frame, (int((x7 + fx[6]) / z7), int((y7 + fy[6]) / z7)), (int((x8 + fx[7]) / z8), int((y8 + fy[7]) / z8)),
             (0, 255, 0), 5)
    cv2.circle(frame, (int((x1 + fx[0]) / z1), int((y1 + fy[0]) / z1)), 5, (255, 0, 0), -1)
    cv2.circle(frame, (int((x2 + fx[1]) / z2), int((y2 + fy[1]) / z2)), 5, (255, 0, 0), -1)
    cv2.circle(frame, (int((x3 + fx[2]) / z3), int((y3 + fy[2]) / z3)), 5, (255, 0, 0), -1)
    cv2.circle(frame, (int((x4 + fx[3]) / z4), int((y4 + fy[3]) / z4)), 5, (255, 0, 0), -1)
    cv2.circle(frame, (int((x5 + fx[4]) / z5), int((y5 + fy[4]) / z5)), 5, (255, 0, 0), -1)
    cv2.circle(frame, (int((x6 + fx[5]) / z6), int((y6 + fy[5]) / z6)), 5, (255, 0, 0), -1)
    cv2.circle(frame, (int((x7 + fx[6]) / z7), int((y7 + fy[6]) / z7)), 5, (255, 0, 0), -1)
    cv2.circle(frame, (int((x8 + fx[7]) / z8), int((y8 + fy[7]) / z8)), 5, (255, 0, 0), -1)
        ...
        if corners:
            ...
            for i in range(0, len(corners)):
                ...
                if not (0 in ar_tag_corners):
                    ...
                    if flag:
                        if angle_value == 0:
                            cube_corners = ar_tag_corners
                        elif angle_value == 90:
                            cube_corners = [ar_tag_corners[2], ar_tag_corners[0], ar_tag_corners[3], ar_tag_corners[1]]
                        elif angle_value == -90:
                            cube_corners = [ar_tag_corners[1], ar_tag_corners[3], ar_tag_corners[0], ar_tag_corners[2]]
                        elif angle_value == 180:
                            cube_corners = [ar_tag_corners[3], ar_tag_corners[2], ar_tag_corners[1], ar_tag_corners[0]]
                        # Calculate homography for cube from camera to world frame
                        H1 = homography([[0, 0], [0, 199], [199, 0], [199, 199]], cube_corners)
                        # Calculate projection matrix as per camera calibrations
                        P = projection_matrix(H1, calibration_matrix, len(corners))
                    # Draw cube
                    draw(frame, P, len(corners))
